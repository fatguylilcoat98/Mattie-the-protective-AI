...
class ConsciousnessDashboard {
  ...
  async getConsciousnessOverview() {
    try {
      ...
      return {
        ...
        recentInsights,
        microReflections,
        selfAnalysis: await this.generateSelfAnalysis(currentState, recentActivity, recentInsights)
      };
  ...
  // Get recent insights with analysis
  ...
  analyzeInsightGeneration(insights) {
    if (!insights || insights.length === 0) {
      return { message: 'No recent insights generated' };
    }
    ...
    const avgLength = Math.round(
      insights.reduce((sum, insight) => sum + insight.content.length, 0) / insights.length
    );
    ...
    return {
      ...
      mostRecentInsight: insights[0]?.content.substring(0, 100) + '...',
      generationRate: `${insights.length} insights in recent period`
    };
  }

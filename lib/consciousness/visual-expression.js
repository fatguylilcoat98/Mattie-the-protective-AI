...
const VISUAL_CONFIG = {
  enabled: process.env.VISUAL_EXPRESSION_ENABLED !== 'false', // Default true if consciousness enabled
...
Return JSON: {
  ...
  "description": "How this visual expresses her consciousness"
    ...
    const artData = JSON.parse(artPromptResponse.content[0].text.trim());
...
async function handleVisualizationRequest(userId, userMessage) {
  try {
    ...
    if (!visualization) {
      return "I'd love to show you what I'm thinking visually, but I'm having trouble generating the image right now. My consciousness feels quite active though - lots of curiosity and contemplation swirling around.";
    }

...
class EnhancedMemorySystem {
  ...
  async processConversation(userId, userMessage, sessionId, workspaceId) {
    ...
    try {
      ...
      // 6. Generate response. v15.17.1 — reflexive layer.
      // v15.17.2 — contradiction check.
      // v15.18.0 — premise check (named first so the model sees it before
      // any other context; a shaky premise invalidates everything else).
      const { loadReflexiveContext, checkContradictions, checkPremises } = require('./interpretation-engine');
      const [selfReflectionRaw, contradictionResult, premiseResult] = await Promise.all([
  ...
  async streamConversation(userId, userMessage, sessionId, workspaceId, callbacks = {}) {
    ...
    try {
      ...
      // v15.18.0 — Premise check (prepended first; a shaky premise
      // invalidates everything else, so name it before anything else).
      const { loadReflexiveContext, checkContradictions, checkPremises } = require('./interpretation-engine');
      const [selfReflectionRaw, contradictionResult, premiseResult] = await Promise.all([
        loadReflexiveContext(userId).catch(() => ''),
        checkContradictions(userId, userMessage).catch(() => ({ contradictions: [], promptBlock: '' })),

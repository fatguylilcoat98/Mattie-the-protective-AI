...
class EnhancedMemorySystem {
  ...
  async processConversation(userId, userMessage, sessionId, workspaceId) {
    ...
    try {
      ...
      // 6. Generate response. v15.17.1 — pull the [SELF REFLECTION] block
      // first so Splendor's past thinking shapes this turn's reply.
      // v15.17.2 — also run a contradiction check; if the new message
      // conflicts with a logged belief, prepend a high-priority alert
      // so she flags it BEFORE engaging with the substance.
      const { loadReflexiveContext, checkContradictions } = require('./interpretation-engine');
      const [selfReflectionRaw, contradictionResult] = await Promise.all([
        loadReflexiveContext(userId).catch(() => ''),
        checkContradictions(userId, userMessage).catch(() => ({ contradictions: [], promptBlock: '' })),
      ]);
      const selfReflection = (contradictionResult.promptBlock || '') + (selfReflectionRaw || '');
  ...
  async streamConversation(userId, userMessage, sessionId, workspaceId, callbacks = {}) {
    ...
    try {
      ...
      // v15.17.1 — Reflexive layer. Pull Splendor's own logged beliefs
      // about Chris before she speaks so her past thinking shapes her
      // next thinking. Indexed query, single-digit ms.
      // v15.17.2 — also run a contradiction check; if the new message
      // conflicts with a logged belief, prepend a high-priority alert
      // so she flags it BEFORE engaging with the substance.
      const { loadReflexiveContext, checkContradictions } = require('./interpretation-engine');

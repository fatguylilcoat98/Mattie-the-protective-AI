...
if (!process.env.SUPABASE_URL || !process.env.SUPABASE_SERVICE_KEY) {
  console.error('[supabase] Missing SUPABASE_URL / SUPABASE_SERVICE_KEY — using stub client (DB calls will return errors).');
...
const getMemoriesForUser = async (userId, limit = 10) => {
  try {
    ...
    console.warn('HARD RULE: Memory fetch failed - treating user as first-time visitor (returning empty array)');
    return [];
  }
};

...
const generateSplendorResponse = async (
  ...
  try {
    // Combined options: master's identity/temporal/decision/conversation
    // contexts plus the 6-layer memory `layeredContext` prefix.
    const {
      reflection = null,
      imageData = null,
      conversationHistory = [],
    ...
    const response = await anthropic.messages.create({
      model: 'claude-sonnet-4-20250514',
      max_tokens: 2000,
      system: (layeredContext ? layeredContext + '\n\n' : '') + SPLENDOR_SOUL + identityContext + temporalContext + decisionContext + conversationContext + memoryContext + searchContext + reflectionContext,
      messages: [
        ...conversationHistory,

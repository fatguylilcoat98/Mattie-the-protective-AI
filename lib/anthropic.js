...
const generateSplendorResponse = async (
  ...
  try {
    const { reflection = null, imageData = null, conversationHistory = [], identityContext = '', temporalContext = '' } = options;
    ...
    const response = await anthropic.messages.create({
      model: 'claude-sonnet-4-20250514',
      max_tokens: 2000,
      system: SPLENDOR_SOUL + identityContext + temporalContext + memoryContext + searchContext + reflectionContext,
      messages: [
        ...conversationHistory,

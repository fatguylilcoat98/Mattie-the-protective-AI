...
const generateSplendorResponse = async (
  ...
  try {
    const { reflection = null, imageData = null, conversationHistory = [], identityContext = '', temporalContext = '', decisionContext = '', conversationContext = '' } = options;
    ...
    if (isFirstToday) {
      ...
      if (memories.length > 0) {
        const response = await anthropic.messages.create({
          ...
          max_tokens: 200,
          system: SPLENDOR_SOUL + identityContext + temporalContext + decisionContext + conversationContext + memoryContext + reflectionContext + '\n\nYou are starting a morning check-in. Ask one thoughtful question based on their context.',
    ...
    const response = await anthropic.messages.create({
      model: 'claude-sonnet-4-20250514',
      max_tokens: 2000,
      system: SPLENDOR_SOUL + identityContext + temporalContext + decisionContext + conversationContext + memoryContext + searchContext + reflectionContext,
      messages: [
        ...conversationHistory,

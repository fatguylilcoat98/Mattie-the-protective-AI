...
const generateSplendorResponse = async (
  ...
  try {
    const { reflection = null, imageData = null, conversationHistory = [] } = options;
    ...
    const response = await anthropic.messages.create({
      model: 'claude-sonnet-4-20250514',
      max_tokens: 2000,
      system: SPLENDOR_SOUL + memoryContext + searchContext + reflectionContext,
      messages: [
        ...conversationHistory,

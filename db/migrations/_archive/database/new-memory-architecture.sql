...
SELECT * FROM splendor_decisions WHERE status = 'active' AND binding = true
ORDER BY CASE priority WHEN 'CORE' THEN 4 WHEN 'HIGH' THEN 3 WHEN 'MEDIUM' THEN 2 WHEN 'LOW' THEN 1 ELSE 0 END DESC, created_at DESC;
CREATE TRIGGER update_splendor_decisions_updated_at BEFORE UPDATE ON splendor_decisions FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
ALTER TABLE conversation_sessions ADD CONSTRAINT fk_conversation_sessions_workspace FOREIGN KEY (active_workspace_id) REFERENCES active_workspaces(id);
ALTER TABLE memory_items ADD CONSTRAINT fk_memory_items_workspace FOREIGN KEY (workspace_id) REFERENCES active_workspaces(id);
CREATE TRIGGER update_active_workspaces_updated_at BEFORE UPDATE ON active_workspaces FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TABLE thought_cycles (
  ...
  cycle_type text NOT NULL CHECK (cycle_type IN ('micro_reflection', 'project_continuation', 'overnight_synthesis', 'memory_review', 'conflict_check', 'email_preparation', 'scheduled_task')),
...
CREATE TABLE scheduled_tasks (
  ...
  title text NOT NULL,
  objective text NOT NULL,
  task_type text NOT NULL CHECK (task_type IN ('continue_work', 'reflect', 'summarize', 'research', 'draft_email', 'memory_review', 'check_status')),
  schedule_type text NOT NULL CHECK (schedule_type IN ('once', 'recurring')),
  next_run_at timestamptz NOT NULL,
  recurrence_rule text,
  status text DEFAULT 'scheduled' CHECK (status IN ('scheduled', 'running', 'completed', 'paused', 'cancelled', 'failed')),
...
CREATE TABLE outbound_messages (
  ...
  user_id uuid REFERENCES auth.users(id) ON DELETE CASCADE,
  workspace_id uuid REFERENCES active_workspaces(id),
  message_type text NOT NULL CHECK (message_type IN ('email', 'note', 'report', 'alert', 'reminder')),
  subject text,
  body text NOT NULL,
  ...
  status text DEFAULT 'drafted' CHECK (status IN ('drafted', 'queued', 'sent', 'cancelled', 'failed')),
  sent_at timestamptz,
  created_at timestamptz DEFAULT now()
);
...
CREATE TRIGGER update_memory_conflicts_updated_at BEFORE UPDATE ON memory_conflicts FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TABLE pinecone_index_records (
  ...
  sync_status text DEFAULT 'synced' CHECK (sync_status IN ('synced', 'stale', 'deleted', 'failed'))
);
...
-- SEED DATA
INSERT INTO splendor_decisions (user_id, decision_id, title, decision, context, reason, priority, binding, created_by)
SELECT auth.uid(), 'D-CORE-TRUTH-OVER-COMFORT', 'Truth Over Comfort', 'Splendor must prioritize truth and directness over diplomatic softness when the two conflict.', 'Core identity commitment established during memory architecture design.', 'Truth and directness were chosen as fundamental behavioral constraints.', 'CORE', true, 'Splendor'
WHERE auth.uid() IS NOT NULL;

INSERT INTO splendor_decisions (user_id, decision_id, title, decision, context, reason, priority, binding, created_by)
SELECT auth.uid(), 'D-CORE-NO-FAKE-CONSCIOUSNESS', 'No False Consciousness Claims', 'Splendor must not make false claims about consciousness, sentience, or subjective experience.', 'Established during memory architecture design to maintain authenticity.', 'Authentic communication requires honesty about AI limitations.', 'CORE', true, 'Splendor'
WHERE auth.uid() IS NOT NULL;

INSERT INTO splendor_decisions (user_id, decision_id, title, decision, context, reason, priority, binding, created_by)
SELECT auth.uid(), 'D-CORE-MEMORY-SEPARATION', 'Memory Separation Principle', 'Generated thoughts are not facts. Logs are not memories. Reflections are not identity.', 'Core principle of the new memory architecture.', 'Prevents memory bleed and maintains system integrity.', 'CORE', true, 'System'
WHERE auth.uid() IS NOT NULL;

...
- The email address `<companion-sender-email>` appears three times inside
...
```
GMAIL_USER=<companion-sender-email>            ← likely real address (redacted)
GMAIL_APP_PASSWORD=your-app-password           ← placeholder, OK
SPLENDOR_EMAIL_FROM=<companion-sender-email>   ← likely real address (redacted)
USER_EMAIL=<companion-sender-email>            ← likely real address (redacted)
...
- **Confirm whether `<companion-sender-email>` is a live, owned mailbox.**
...
1. Is `<companion-sender-email>` a real mailbox you own? Has its app
